...
exports.getSearhCab = async (req, res) => {
	...
	const seating_capacity = (typeof req.body.seating_capacity != 'undefined') ? Number(req.body.seating_capacity) : 0;
	const luggage = (typeof req.body.luggage != 'undefined') ? Number(req.body.luggage) : 0;
...
const getFareDataByCityIdPackageId = async (arrobj) => {
	...
	for (var i = 0; i < faredata.length; i++) {
		...
		let localpkgFare = await FARE.getPackageFareByPackageId(base_vehicle_id, local_pkg_id);
		...
		if (typeof localpkgData !== 'undefined' && localpkgData != '') {
			...
			let getmodeFare = await FARE.getFareByPackagemodeId(master_packge_mode_id, base_vehicle_id);
